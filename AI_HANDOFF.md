...
- `app/src/main/java/com/blackpirateapps/brownpaper/ui`
  ...
  - `reader`: Native reading screen with annotation-aware selectable text rendering. Supports custom fonts (Merriweather, Lora, Fira Sans, Inter), explicit reader themes, dropdown font weights, persisted content width, article metadata, inline highlights, notes, and an annotation list.
...
- **Article cards:** Homepage cards only render media when an article has a hero image. If Coil cannot load the image, the card hides the media area instead of leaving a blank image block. Quick actions are available on each card for favorite, archive/unarchive, mark as read, and delete. Read/archived cards are visually muted, and tag feeds include read articles instead of filtering them out.
...
- **Annotations:** Articles support offline-first annotations with highlight color, quote, note text, edit/delete, copy quote, search within annotations, tap-to-edit highlights, and jump-to-highlight from the annotation sheet. Reader paragraphs are rendered through selectable Android `TextView`s so the platform text-selection toolbar can show copy/share/search-style actions; BrownPaper adds a `Highlight` action to that toolbar to open the annotation editor for the selected text. Reader highlights use the reader palette rather than the app theme, and reader images hide themselves if loading fails.
...
- **wallabag sync:** Room is at schema version 5. Articles carry wallabag entry metadata and local/remote timestamps, `wallabag_sync_operations` records pending local updates, `wallabag_delete_operations` records remote entry deletes after local deletion, and `wallabag_annotation_sync_operations` records pending annotation creates/updates/deletes. Local saves, likes, archive changes, tag edits, deletes, and annotation changes queue sync work and schedule WorkManager. Annotation create/update payloads use scalar `quote` and `text` fields with a `ranges` collection to match wallabag's Symfony form handling.
...
- Attempted local verification after the wallabag annotation payload and reader-selection changes, but the repo has no Gradle wrapper and `gradle` is not installed in the environment.
