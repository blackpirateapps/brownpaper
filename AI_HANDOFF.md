...
- **wallabag sync:** Room is at schema version 3. Articles now carry wallabag entry metadata and local/remote timestamps, and a `wallabag_sync_operations` table records pending local work. Local saves, likes, archive changes, and tag edits queue sync operations and schedule WorkManager.
- **Sync policy:** Sync pulls paged wallabag entries with `detail=full`, links by URL before creating remote duplicates, pushes local unsynced articles, and patches starred/archived/tags. Deletes remain local-only.
...
- Verification was not run after the wallabag implementation in this handoff update; the repository does not currently include a Gradle wrapper, and the user asked not to verify here.
